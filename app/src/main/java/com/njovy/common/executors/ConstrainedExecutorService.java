...
package com.njovy.common.executors;


import com.njovy.common.logging.OLog;
...
public class ConstrainedExecutorService extends AbstractExecutorService {
  ...
  public void execute(Runnable runnable) {
    ...
    if ((queueSize > maxSize) && mMaxQueueSize.compareAndSet(maxSize, queueSize)) {
      OLog.v(TAG, "%s: max pending work in queue = %d", mName, queueSize);
  ...
  private void startWorkerIfNeeded() {
    ...
    while (currentCount < mMaxConcurrency) {
      ...
      if (mPendingWorkers.compareAndSet(currentCount, updatedCount)) {
        // Start a new worker.
        OLog.v(TAG, "%s: starting worker %d of %d", mName, updatedCount, mMaxConcurrency);
        mExecutor.execute(mTaskRunner);
        break;
      }
      // else: compareAndSet failed due to race; snapshot the new count and try again
      OLog.v(TAG, "%s: race in startWorkerIfNeeded; retrying", mName);
    ...
    public void run() {
      try {
        ...
        if (runnable != null) {
          ...
          OLog.v(TAG, "%s: Worker has nothing to run", mName);
        }
      } finally {
        ...
        if (!mWorkQueue.isEmpty()) {
          ...
          OLog.v(TAG, "%s: worker finished; %d workers left", mName, workers);
        }
      }
    }
  }
